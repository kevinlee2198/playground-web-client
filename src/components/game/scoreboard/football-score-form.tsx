...
interface FootballScoreFormProps {
...
const footballScoreSchema = z.object({
  scoreA: z
    .number()
...
function extractScore(participant: GameParticipantDetail): number {
  ...
  if (metadata?.__typename === "FootballParticipantMetadata") {
    return metadata.score;
  }
  return 0;
}

export function FootballScoreForm({
  participantA,
  participantB,
  nameA,
  nameB,
  ...
  const form = useForm({
    ...
    validators: {
      onBlur: ({ value }) => {
        const result = footballScoreSchema.safeParse(value);
      ...
      const entries: UpdateParticipantScoreEntry[] = [
        {
          id: participantA.id,
          isTeam,
          metadata: { football: { score: value.scoreA } },
        },
        {
          id: participantB.id,
          isTeam,
          metadata: { football: { score: value.scoreB } },
        },
      ];
