...
import type { FootballDefensiveStatsNode } from "@/lib/types/stats/football";
...
import { FootballDefensiveStatsForm } from "./football-defensive-stats-form";
...
export function FootballDefensiveStatsTable({
  ...
  function handleAddPlayerStats() {
    ...
    startTransition(async () => {
      const result = await saveFootballDefensiveStats({
  ...
  const columns: ColumnDef<FootballDefensiveStatsNode>[] = useMemo(() => {
    ...
    function highlightableStatColumn(
      stat: HighlightableStat,
    ): ColumnDef<FootballDefensiveStatsNode> {
      return {
        accessorKey: stat,
    ...
    function plainStatColumn(
      key: keyof FootballDefensiveStatsNode & string,
    ): ColumnDef<FootballDefensiveStatsNode> {
      return {
        accessorKey: key,
        header: t(key),
    ...
    return [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          if (solo == null && ast == null) {
            return <span className="tabular-nums">-</span>;
          }
          return (
            <span className="tabular-nums">{(solo ?? 0) + (ast ?? 0)}</span>
          );
        },
      },
      ...HIGHLIGHTABLE_STATS.map(highlightableStatColumn),
      plainStatColumn("assistedTackles"),
      plainStatColumn("tacklesForLoss"),
      plainStatColumn("passesDefended"),
      plainStatColumn("interceptionReturnYards"),
      plainStatColumn("interceptionReturnTouchdowns"),
      plainStatColumn("fumbleRecoveries"),
      plainStatColumn("fumbleReturnYards"),
      plainStatColumn("fumbleReturnTouchdowns"),
      plainStatColumn("safeties"),
      ...(canEdit
        ? [
            {
              id: "actions",
              ...
              cell: ({
                ...
                row: { original: FootballDefensiveStatsNode };
              }) => (
                <Button
                  variant="ghost"
                  size="icon"
